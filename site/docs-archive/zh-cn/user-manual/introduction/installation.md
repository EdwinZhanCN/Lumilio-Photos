...
每个 release 都附带校验文件。下载后先校验，再安装：

```bash
shasum -a 256 Lumilio-Photos-<版本>-macos-arm64.dmg
```

把输出与 release 页面公布的 SHA-256 校验值比对；**不一致就停止安装**，删除下载的文件并从官方 release 重新下载。

当前 macOS 应用尚未经过 Apple 公证。校验无误后第一次启动时，系统仍可能阻止打开：
...
只对**从项目 GitHub Releases 下载且校验值匹配的单个安装文件**执行这一步。不要关闭全局安全机制；不需要时，从“应用程序”删除应用、并删除下载的 DMG。
...
当前 Windows 安装程序尚未进行 Authenticode 签名。下载后先校验：

```powershell
Get-FileHash -Algorithm SHA256 .\Lumilio-Photos-<版本>-windows-amd64-setup.exe
```

把输出与 release 页面公布的 SHA-256 校验值比对；不一致就删除文件并从官方 release 重新下载。如果 SmartScreen 显示“Windows 已保护你的电脑”，请确认文件来自项目 GitHub Releases 且校验值匹配，然后选择 **更多信息 → 仍要运行**。只对校验过的单个安装文件执行这一步，不要关闭全局安全机制。
...
默认 Compose 使用 Linux host network，让 Server 可以发现局域网中的 Lumen 节点，无需添加 `ports`。这是 Docker 版 Lumen 自动发现的支持边界；如果平台不能创建 host network 项目，当前官方路径不支持用该平台部署 Docker 版 Lumen Hub。具体流程见 [Lumen Intelligence](../features/lumen-intelligence)。
:::
...
- [Lumen Intelligence](../features/lumen-intelligence)：按需启用本地或局域网媒体理解能力；
