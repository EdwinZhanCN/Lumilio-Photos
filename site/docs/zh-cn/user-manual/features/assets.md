...
单击页面右下角的搜索按钮，输入框随即展开。输入关键词后，资源库将自动展示搜索结果。
...
- **标记喜欢** — 切换资源的喜欢状态
- **分享** — 为当前资源创建公开分享链接
- **导出** — 打开导出弹窗，下载原文件或导出为其他格式
- **删除** — 将资源移入回收站，操作前弹出确认对话框

分享链接的创建、下载权限和管理方式见[分享](/zh-cn/user-manual/features/sharing)。
...
| **连拍** | 优先依据 EXIF 连拍标识归组；缺少标识时，仅对至少 3 张、文件名连续且拍摄间隔不超过 1 秒的序列进行保守识别 | 已实现 |
| **编辑版本** | 原始资源及其带连续版本编号的编辑迭代自动关联 | 已实现 |
...
> Lumilio Agent 是一个独立功能，此处仅提及入口位置。管理员可在[设置 → AI](/zh-cn/user-manual/features/settings#ai) 中配置 Agent 的服务商和模型。详细用法见[Lumilio Agent](/zh-cn/user-manual/features/lumilio)。
