...
description: "理解读取工具、改变图库的工具、确认流程与当前撤销边界。"
...
当前四个会改变图库的 Agent 工具——批量喜欢、创建相册、添加到相册和添加标签——都在效果策略中声明需要确认并标记为可逆。执行前，界面会展示拟执行效果；只有用户确认后，服务才提交事务并写入效果回执。
...
- 操作完成后在资源库中验证结果。
- 当前没有经过代码验证的通用“撤销 Agent 操作”端点或界面；发生误操作时，应使用图库现有编辑能力人工恢复，并保留效果 ID。

读取工具仍可能把图库元数据摘要用于模型推理；权限控制不等于外部数据零披露。相关边界见[Agent 隐私](./agent-privacy.md)。
