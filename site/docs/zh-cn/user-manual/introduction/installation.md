...
::: tip 更新
有新版本时,菜单栏/托盘菜单会显示**有新版本**——点击即可下载对应平台的安装包
(macOS 为 `.dmg`,Windows 为 `setup.exe`),再覆盖安装即可。照片库与数据库不受影响。
首次设置(或之后在控制面板)里可选择**下载地区**;选「中国大陆」时安装包走
GitHub 镜像。该选项与应用内地图等地区设置相互独立。
:::
...
::: tip 更新
有新版本时,托盘菜单会显示**有新版本**——点击下载新的 `setup.exe` 并覆盖安装即可。
照片库与数据库不受影响。**下载地区**(中国大陆 / 其他)决定安装包是否走
GitHub 镜像,与应用内地图等地区设置相互独立。
:::
