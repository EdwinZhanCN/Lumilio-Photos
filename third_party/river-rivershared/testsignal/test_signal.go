...
const testSignalWaitTimeout = 10 * time.Second
...
func (s *TestSignal[T]) WaitOrTimeout() T {
	...
	case val := <-s.internalChan:
		return val
	case <-time.After(testSignalWaitTimeout):
		s.tb.Errorf("timed out waiting on test signal after %s", testSignalWaitTimeout)
	}
