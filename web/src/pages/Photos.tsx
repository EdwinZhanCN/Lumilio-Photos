...
type PhotosBySizeRange = {
    [sizeRange: string]: Photo[];
};

const groupPhotosByDate = (photos: Photo[]): PhotosByDate => {
    return photos.reduce<PhotosByDate>((acc, photo) => {
...
const groupPhotosByTags = (photos:Photo[]):PhotosByTag => {
    return photos.reduce<PhotosByTag>((acc, photo) => {
...
const groupPhotosByType = (photos:Photo[]) :PhotosByType=> {
    return photos.reduce<PhotosByType>((acc, photo) => {
...
// 新增分组函数：按大小范围
const groupPhotosBySizeRange = (photos:Photo[]):PhotosBySizeRange => {
    // 定义大小分组
    const sizeGroups = [
        { name: ">1000MB", condition: (mb:number) => mb > 1000 },
        { name: "100MB - 1000MB", condition: (mb:number) => mb >= 100 && mb <= 1000 },
        { name: "1MB - 100MB", condition: (mb:number) => mb >= 1 && mb < 100 },
        { name: "<1MB", condition: (mb:number) => mb < 1 },
    ];

    return photos.reduce<PhotosBySizeRange>((acc, photo) => {
...
// 渲染组件
const Photos = () => {
    const [currentGrouping, setCurrentGrouping] = useState<keyof typeof groupingFunctions>("date"); // 当前分组方式
...
// 全屏照片轮播组件
const FullScreenCarousel = ({ photos, initialSlide }:FullScreenCarouselProps) => {
    ...
    const handleShow = () => {
        ...
        // Force a reflow
        if (controls) void controls.offsetHeight;
        if (toolbar) void toolbar.offsetHeight;
...
const PhotoInfo = ({ photo, onClose }:PhotoInfoProps) => {
    return (
        <div
