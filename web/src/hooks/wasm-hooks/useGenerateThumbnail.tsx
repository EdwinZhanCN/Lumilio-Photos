...
type UseGenerateThumbnailProps = {
    setGenThumbnailProgress: React.Dispatch<React.SetStateAction<{
...
export const useGenerateThumbnail = ({
    ...
    generatePreviews: (files: File[]) => Promise<void | { index: number; url: string }[] | Error>
} => {
     ...
     */
    const generatePreviews = useCallback(async (files: File[]):Promise<void | { index: number; url: string }[] | Error> => {
        ...
        try {
            ...
            setGenThumbnailProgress(prev => prev ? {
                ...prev,
                error: errorMessage,
                failedAt: Date.now(),
            } : null);
    ...
    }, [wasmReady, workerClientRef, setPreviews, setIsGenThumbnails, setGenThumbnailProgress, showMessage]);
