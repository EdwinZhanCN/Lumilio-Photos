...
import { globalPerformancePreferences, PerformanceProfile } from './performancePreferences';
...
export class SmartBatchSizer {
  ...
  getOptimalBatchSize(
    ...
    let batchSize = this.currentBatchSizes.get(operationType) || this.getInitialBatchSize(operationType);
    ...
    batchSize = this.applyPerformancePreferences(batchSize, operationType);
    
    // Apply priority adjustments (after preferences to respect user choice)
    if (priority >= ProcessingPriority.HIGH && globalPerformancePreferences.shouldPrioritizeUserOperations()) {
  ...
  private applyDeviceConstraints(batchSize: number, operationType: string): number {
    ...
    const memoryMultiplier = globalPerformancePreferences.getMemoryConstraintMultiplier();
   ...
   */
  private applyPerformancePreferences(batchSize: number, operationType: string): number {
