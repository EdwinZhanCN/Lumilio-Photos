...
describe("SmartBatchSizer", () => {
  ...
  describe("getOptimalBatchSize", () => {
    it("should return appropriate batch size for thumbnail operation", () => {
      const batchSize = batchSizer.getOptimalBatchSize(
        "thumbnail",
        100,
    ...
    it("should increase batch size for high priority operations", () => {
      const normalBatchSize = batchSizer.getOptimalBatchSize(
        "thumbnail",
        100,
        ProcessingPriority.NORMAL,
      );
      batchSizer.resetMetrics(); // Reset to get fresh calculation
      const highBatchSize = batchSizer.getOptimalBatchSize(
        "thumbnail",
        100,
        ProcessingPriority.CRITICAL,
      );
  ...
  describe("recordMetrics and adaptation", () => {
    it("should adapt batch size based on slow processing metrics", () => {
      const initialBatchSize = batchSizer.getOptimalBatchSize(
        "thumbnail",
        100,
        ProcessingPriority.NORMAL,
      );
      ...
      const adaptedBatchSize = batchSizer.getOptimalBatchSize(
        "thumbnail",
        100,
        ProcessingPriority.NORMAL,
      );
