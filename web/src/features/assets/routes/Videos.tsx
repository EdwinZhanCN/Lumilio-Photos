...
function Videos() {
  ...
  const flatAssets = useMemo(() => {
    const baseAssets = getFlatAssetsFromGrouped(finalGroupedVideos);
  ...
  return (
    <div>
      ...
      {isFetching && allAssets.length === 0 ? (
        ...
        <JustifiedGallery
          groupedPhotos={finalGroupedVideos}
      ...
      {!hasNextPage && allAssets.length > 0 && (
        <div className="text-center p-4 text-gray-500">{t("assets.videos.end_of_results")}</div>
      )}
        ...
        (flatAssets.length > 0 ? (
            ...
            {isLocatingAsset && (
              <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center">
                <div className="text-white text-center bg-black/50 backdrop-blur-sm rounded-2xl p-8 max-w-md">
                  ...
                  <p className="text-lg font-medium mb-2">{t("assets.videos.locating_asset")}</p>
                  {hasNextPage && !isFetching && !isFetchingNextPage ? (
                    <p className="text-sm text-gray-300">
                      {t("assets.videos.loading_more_data")}
                    </p>
                  ) : (
                    <p className="text-sm text-gray-300">
                      {t("assets.videos.asset_not_available")}
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center">
            <div className="text-white text-center">
              <div className="loading loading-spinner loading-lg mb-4"></div>
              <p>{t("assets.videos.loading_assets")}</p>
            </div>
          </div>
        ))}
    </div>
