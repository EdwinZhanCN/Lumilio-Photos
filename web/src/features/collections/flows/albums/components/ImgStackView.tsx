...
function ImgStackView({
  ...
  return (
    ...
    >
      {/* Back card */}
      <div
        className={`absolute inset-0 translate-x-2 translate-y-2 -rotate-2 rounded-2xl border border-base-300/60 bg-gradient-to-br from-base-200 to-base-100 shadow-lg transition-all duration-300 ease-out 
      ...
      {/* Middle card */}
      <div
        className={`absolute inset-0 translate-x-1 translate-y-1 rotate-1 rounded-2xl border border-base-300/70 bg-gradient-to-br from-primary/10 via-base-100 to-secondary/10 shadow-xl transition-all duration-300 ease-out
      ...
      {/* Top card */}
      <div
        className={`relative flex size-full items-center justify-center overflow-hidden rounded-2xl border shadow-2xl transition-all duration-300
        ${
          isSelected
      ...
      >
        ...
        <div
          className={`pointer-events-none absolute -left-8 -top-8 h-24 w-24 rounded-full bg-gradient-to-br from-primary/30 to-transparent blur-2xl transition-opacity duration-300 ${selectionOpacity}`}
        />
        <div
          className={`pointer-events-none absolute -right-8 -bottom-10 h-28 w-28 rounded-full bg-gradient-to-tr from-secondary/30 to-transparent blur-2xl transition-opacity duration-300 ${selectionOpacity}`}
        />
        ...
        {hasCover && !imageError ? (
          ...
          <GalleryVerticalEnd
            className={`size-10 transition-colors duration-300 ${isSelected ? "text-primary" : "text-base-content/80"}`}
          />
        )}
