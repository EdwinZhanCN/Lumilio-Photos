...
faking fields just to satisfy them. [PublicShareGrid](./components/PublicShareGrid.tsx) and
[PublicShareLightbox](./components/PublicShareLightbox.tsx) are small, purpose-built components instead;
[shareUrls](./utils/shareUrls.ts) builds token-scoped media URLs (no media-token query
param, unlike `assetUrls`).

## Creation entry points

[createShareSelectedBulkAction](./utils/shareBulkAction.tsx) is a reusable "Share selected" bulk
action wired into every gallery that supports multi-select (Assets, Liked,
Album, Person, Utility classifier) — it opens [CreateShareLinkModal](./components/CreateShareLinkModal.tsx)
...
[CreateShareLinkModal](./components/CreateShareLinkModal.tsx)'s success state, right after creation.
[SharedLinks](./routes/SharedLinks.tsx) intentionally has no "copy" action on existing rows;
