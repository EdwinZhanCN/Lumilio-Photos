...
This document models the `uploadReducer` transitions in `web/src/features/upload/state/reducer.ts` using the actions defined beside the feature context in `web/src/features/upload/state/context.ts`. It focuses on reducer-driven state changes (not UI flow).
