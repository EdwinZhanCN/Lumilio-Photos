...
[useMusicTracks](./api/useMusic.ts), [useMusicAlbums](./api/useMusic.ts), [useMusicArtists](./api/useMusic.ts),
[useMusicPlaylists](./api/useMusic.ts), and their detail counterparts. URL parameters
select the browse view and search state. [MusicPlayerProvider](./state/MusicPlayerProvider.tsx) owns
...
[MusicLibraryFlow](./flows/browse/MusicLibraryFlow.tsx) is the real-data browse surface for
...
[MusicTrackRow](./components/MusicTrackRow.tsx) can start a server-backed snapshot, so playing a
...
[MusicAgentResult](./components/MusicAgentResult.tsx) hydrates owner/thread-scoped refs inside Lumilio chat.
...
[MusicPlayerDock](./components/MusicPlayerDock.tsx) is composed once by the application shell beside
