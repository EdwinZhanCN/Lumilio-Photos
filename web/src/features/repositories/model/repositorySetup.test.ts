...
import { validateRepositoryDirectoryName, validateRepositoryName } from "./repositorySetup";

describe("validateRepositoryName", () => {
  it.each(["家庭媒体", "Family Media 2026", "Family.Media (2026)!"])(
...
describe("validateRepositoryDirectoryName", () => {
  it.each(["家庭媒体", "Family Media 2026", "Media_2026-Archive"])(
    "accepts portable storage folder %s",
    (value) => {
      expect(validateRepositoryDirectoryName(value)).toBeNull();
    },
  );
  ...
  it.each([
    ...
    expect(validateRepositoryDirectoryName(value)).toBe(error);
  });
});
