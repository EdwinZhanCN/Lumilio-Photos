...
export default function NativeHostActionModal({
  ...
  const persistenceScope = storageLocationId || repositoryId || "new";
  ...
  useEffect(() => {
    ...
    const recovered = unfinishedQuery.data.find(
      (candidate) =>
        candidate.kind === kind &&
        (candidate.storage_location_id ?? "") === (storageLocationId ?? "") &&
        (candidate.repository_id ?? "") === (repositoryId ?? ""),
    );
    if (recovered) setAction(recovered);
  }, [action, isOpen, kind, repositoryId, storageLocationId, unfinishedQuery.data]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      queryClient.invalidateQueries({ queryKey: ["get", "/api/v1/storage/view"] }),
