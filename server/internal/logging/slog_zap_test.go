...
func TestSlogZapHandlerMapsLevelsAndAttrs(t *testing.T) {
	...
	logger.ErrorContext(context.Background(), "bridge error", slog.String("job_kind", "enrich_asset"), slog.Int64("job_id", 12))

	entries := observed.All()
	if assert.Len(t, entries, 1) {
		assert.Equal(t, zapcore.ErrorLevel, entries[0].Level)
		assert.Equal(t, "bridge error", entries[0].Message)
		assert.Equal(t, "enrich_asset", entries[0].ContextMap()["job_kind"])
