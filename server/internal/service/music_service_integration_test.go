...
func musicTestCatalog(t *testing.T) (*db.DB, *musicService, uuid.UUID) {
	...
	storageLocationID, repositoryID := uuid.New(), uuid.New()
 ...
 INSERT INTO storage_locations (storage_location_id, name, path, kind, created_at, updated_at) VALUES (?, 'Root', '/music', 'default', 1, 1);
 INSERT INTO repositories (repo_id, name, path, created_at, updated_at, default_owner_id, role, storage_location_id) VALUES (?, 'Music', '/music/library', 1, 1, 1, 'primary', ?);`, storageLocationID, repositoryID, storageLocationID)
	require.NoError(t, err)
...
func TestMusicRatingsAndPlaylistCoverProjection(t *testing.T) {
	ctx := context.Background()
	catalog, service, repositoryID := musicTestCatalog(t)
	first := seedMusicTrack(t, catalog, service, repositoryID, `{"title":"First"}`)
	second := seedMusicTrack(t, catalog, service, repositoryID, `{"title":"Second"}`)
	_, err := catalog.SQL.ExecContext(ctx, `UPDATE assets SET rating = 4 WHERE asset_id = ?`, first)
	require.NoError(t, err)
	track, err := service.GetTrack(ctx, 1, first)
	require.NoError(t, err)
	require.NotNil(t, track.Rating)
	require.EqualValues(t, 4, *track.Rating)
	tracks, err := service.ListTracks(ctx, 1, "First", "", "", false, 50, 0)
	require.NoError(t, err)
	require.EqualValues(t, 4, *tracks.Items[0].Rating)
	playlist, err := service.CreatePlaylist(ctx, 1, "Cover selection", "")
	require.NoError(t, err)
	_, err = service.AddPlaylistEntry(ctx, 1, playlist.PlaylistID, first, "first", nil, playlist.Revision)
	require.NoError(t, err)
	playlist, err = service.GetPlaylist(ctx, 1, playlist.PlaylistID)
	require.NoError(t, err)
	_, err = service.AddPlaylistEntry(ctx, 1, playlist.PlaylistID, second, "second", nil, playlist.Revision)
	require.NoError(t, err)
	_, err = catalog.SQL.ExecContext(ctx, `INSERT INTO thumbnails (asset_id, size, storage_path, mime_type, created_at) VALUES (?, 'medium', 'cover.jpg', 'image/jpeg', 1)`, second)
	require.NoError(t, err)
	playlist, err = service.GetPlaylist(ctx, 1, playlist.PlaylistID)
	require.NoError(t, err)
	require.Equal(t, second.String(), playlist.CoverAssetID)
	playlists, err := service.ListPlaylists(ctx, 1, 50, 0)
	require.NoError(t, err)
	require.Equal(t, second.String(), playlists.Items[0].CoverAssetID)
	...
	playlist, err = service.GetPlaylist(ctx, 1, playlist.PlaylistID)
	require.NoError(t, err)
	require.Empty(t, playlist.CoverAssetID)
}
