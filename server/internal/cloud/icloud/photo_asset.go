...
func (r *PhotoAsset) LocalPath(outputDir string, size PhotoVersion, fileStructure string, livePhoto bool) string {
	...
	case "name":
		...
		// name already contains ext, so return directly
		if size == PhotoVersionOriginal || size == "" {
			return filepath.Join(outputDir, name)
		}
		base := name[:len(name)-len(ext)]
		return filepath.Join(outputDir, base+"_"+string(size)+ext)
	default:
...
// Fingerprint returns the original file fingerprint (etag equivalent).
func (r *PhotoAsset) Fingerprint() string {
	return r._masterRecord.Fields.ResOriginalFingerprint.Value
}

// MIMEType returns the original file MIME type.
func (r *PhotoAsset) MIMEType() string {
	return r._masterRecord.Fields.ResOriginalFileType.Value
}

// IsDeleted reports whether the asset has been deleted in iCloud.
func (r *PhotoAsset) IsDeleted() bool {
