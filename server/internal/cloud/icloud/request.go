...
func (r *Client) request(req *rawReq) (string, error) {
	text, _, _, err := r.doRequest(req)
	return text, err
}

func (r *Client) requestWithStatus(req *rawReq) (string, int, error) {
	text, _, status, err := r.doRequest(req)
	return text, status, err
}

func (r *Client) requestStream(req *rawReq) (io.ReadCloser, error) {
	req.Stream = true
	_, body, _, err := r.doRequest(req)
	return body, err
}

func (r *Client) doRequest(req *rawReq) (string, io.ReadCloser, int, error) {
	status := 0
	...
	if status == http.StatusGone {
		return "", nil, status, fmt.Errorf("%s %s failed, %w", req.Method, req.URL, ErrResourceGone)
	}

	if req.Stream {
		if respErr != nil {
			return "", nil, status, fmt.Errorf("%s %s failed, status %d, err: %s", req.Method, req.URL, status, respErr)
		}
		if req.ExpectStatus != nil && req.ExpectStatus.Len() > 0 && !req.ExpectStatus.Has(status) {
			return "", nil, status, fmt.Errorf("%s %s failed, expect status %v, but got %d", req.Method, req.URL, req.ExpectStatus.String(), status)
		}
	...
	text, err := res.Text()
	if err != nil {
		return text, nil, status, fmt.Errorf("%s %s failed, status %d, err: %s, response text: %s", req.Method, req.URL, status, err, text)
	}

	if err := mayErr([]byte(text)); err != nil {
		return text, nil, status, fmt.Errorf("%s %s failed, status %d, err: %w", req.Method, req.URL, status, err)
	}

	if req.ExpectStatus != nil && req.ExpectStatus.Len() > 0 && !req.ExpectStatus.Has(status) {
		return text, nil, status, fmt.Errorf("%s %s failed, expect status %v, but got %d, response text: %s", req.Method, req.URL, req.ExpectStatus.String(), status, text)
	}

	return text, nil, status, err
}
...
func (r *Client) getAuthHeaders(overwrite map[string]string) map[string]string { //            "Accept": "*/*",
	headers := map[string]string{
		...
		"X-Apple-OAuth-Redirect-URI":       r.homeEndpoint,
	...
	scnt := r.sessionData.Scnt
	if scnt != "" {
		headers["scnt"] = scnt
	}
	sessionID := r.sessionData.SessionID
	if sessionID != "" {
