...
// Authenticate if MFA is required and the getter was not provided at
// construction time.
func (r *Client) SetTwoFACodeGetter(getter TextGetter) {
	r.twoFACodeGetter = getter
}
...
func newClient(option *ClientOption) (*Client, error) {
	...
	if option.Domain == "cn" {
		...
		cli.authRootEndpoint = "https://idmsa.apple.com.cn"
		cli.authEndpoint = "https://idmsa.apple.com.cn/appleauth/auth"
	} else if option.Domain == "com" {
		cli.setupEndpoint = "https://setup.icloud.com/setup/ws/1"
		cli.homeEndpoint = "https://www.icloud.com"
		cli.authRootEndpoint = "https://idmsa.apple.com"
