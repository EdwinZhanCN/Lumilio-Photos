...
-- Enrollment secrets never live in a signed client token. The opaque
-- enrollment_id is the only public handle; the candidate secret remains
-- encrypted in this table until the one-time activation transaction consumes
-- it.
...
-- source_key + geohash is the only result identity in generation 7.
