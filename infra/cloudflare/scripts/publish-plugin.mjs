...
function normalizeManifest(rawManifest, pluginId, version, cdnOrigin) {
  ...
  if (!cdnOrigin && (looksLikePlaceholderUrl(manifest.entries.ui) || looksLikePlaceholderUrl(manifest.entries.runner))) {
    usageAndExit(
      "Manifest entries still point to example.com. Pass --cdn-origin to rewrite runtime URLs for this release.",
    );
  }

  if (!Array.isArray(manifest.permissions) || !manifest.permissions.every((item) => typeof item === "string")) {
  ...
  manifest.signature = {
    keyId: isNonEmptyString(manifest.signature.keyId) ? manifest.signature.keyId : DEFAULT_KEY_ID,
    algorithm: "ECDSA_P256_SHA256",
    value: isNonEmptyString(manifest.signature.value) ? manifest.signature.value : "",
  };
...
function resolveEntryFilePath(sourceDir, entryUrl) {
  ...
  if (!fileName) {
    usageAndExit(`Unable to infer artifact file from entry URL: ${entryUrl}`);
  }
...
function ensureEntryArtifactsExist(sourceDir, entries) {
  ...
  if (!existsSync(runnerPath)) {
    usageAndExit(`Runner entry artifact not found in source dir: ${runnerPath}`);
  }
}

function maybeSignManifest(manifest) {
  const privateKeyPem = process.env.PLUGIN_SIGNING_PRIVATE_KEY_PEM;
  const keyId = process.env.PLUGIN_SIGNING_KEY_ID || manifest.signature?.keyId || DEFAULT_KEY_ID;
...
function main() {
  ...
  const cdnOrigin = args["cdn-origin"] ? normalizeOrigin(args["cdn-origin"]) : null;
  ...
  const manifestInput = JSON.parse(readFileSync(resolvedManifestPath, "utf8"));
  const manifest = normalizeManifest(manifestInput, pluginId, version, cdnOrigin);
