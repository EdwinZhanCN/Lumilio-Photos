...
运行前需要安装 Docker 及 Compose 插件。请将 `LUMILIO_STORAGE` 设置为用于保存媒体库和持久化密钥的宿主机目录。
...
> [!IMPORTANT]
> `LUMILIO_SECRET_KEY` 表示密钥的**文件路径**，而不是密钥原文。服务端会在需要时创建该文件，并使用它派生 JWT 签名密钥和设置加密密钥。发布版 Compose 会将其持久化到 `LUMILIO_STORAGE` 下。
...
`make dev` 会启动位于宿主机 `5433` 端口的 PostgreSQL、位于 `6680` 端口的 API，以及位于 `6657` 端口的 Web 应用。初始化时，本地配置会从 `server/config/server.example.toml` 复制到被 Git 忽略的 `server/config/server.local.toml`。
